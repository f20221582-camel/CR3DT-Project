...
class CR3DTNet(CenterPoint):
    ...
    def __init__(self, late_fusion, img_view_transformer, img_bev_encoder_backbone,
        ...
        # --- NEW ARCHITECTURE: RADAR ADAPTER ---
        # A 1x1 Conv to learn better radar features before fusion
        self.radar_adapter = nn.Sequential(
            nn.Conv2d(18, 18, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(18),
    ...
    def forward_train(self,
        ...
        with context:
            ...
            if self.radar_voxel_encoder:
                # --- APPLY RADAR ADAPTER ---
                # Process the radar features to reduce noise
                clean_radar_feats = self.radar_adapter(radar_feats)
                # Fuse
                fused_feats = torch.cat([fused_feats, clean_radar_feats], dim=1)
