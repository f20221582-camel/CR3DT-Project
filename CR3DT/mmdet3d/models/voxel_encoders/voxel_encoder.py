...
class OccupancyVFE(nn.Module):
    """Simple occupancy feature encoder used in SimpleBEV.

    It simply builds an occupancy grid from the points inside a voxel.

    Args:
        grid_config (dict[List]): lower, upper & interval of the x, y, z dimension of the grid.
    """
    def __init__(self, grid_config, transpose=False, rot=False, rot_angle=0) -> None:
        super(OccupancyVFE, self).__init__()
    ...
    def forward(self, points):
        """Voxelize the points and build an occupancy grid per batch.

        Args:
            points (list[torch.Tensor]): List of point clouds with shape (B, N, point_features).
                                         Lidar features are typically x,y,z,reflectivity.
        
        Returns:
            The occupancy grid with shape (B, 1, Z, Y, X) with B being the batch size.
        """
        ...
        bev_shape = (num_z_bins, num_y_bins, num_x_bins) # (8, 128, 128)
        occupancy_grid = torch.zeros((batch_size,) + bev_shape, dtype=torch.float32, device=points[0].device)
        ...
        for batch_idx in range(batch_size):
            ...
            occupancy_grid[batch_idx, indices[:, 2], indices[:, 1], indices[:, 0]] = 1
